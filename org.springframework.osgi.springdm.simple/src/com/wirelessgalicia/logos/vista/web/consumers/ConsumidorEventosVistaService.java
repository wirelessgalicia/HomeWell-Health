...
import com.wirelessgalicia.logos.vista.web.almacen.AlmacenEventos;
 ...
 *
 */
public class ConsumidorEventosVistaService implements Consumer, Runnable {
	/**
	 * Logger
	 */
	private Logger LOGGER = LoggerFactory.getLogger(ConsumidorEventosVistaService.class.getName());
	 ...
	 */
	public ConsumidorEventosVistaService(BundleContext context, ConsumidorVistaVO consumidor) {
		...
		if (LOGGER.isTraceEnabled())
			...
			try{
				for (Class<?> clazz:clazzes){
					if (clazz.isAssignableFrom(EventoRealizacionPrueba.class) && clazz.isAssignableFrom(value.getClass())){
						EventoRealizacionPrueba evento = (EventoRealizacionPrueba)value;
						if (LOGGER.isDebugEnabled())
							LOGGER.debug("\t\tCONSUMIDOR VISTA (Evento): " +
									wire.getProperties().get(WireConstants.WIREADMIN_PRODUCER_PID) + 
									" = " + evento);
						AlmacenEventos.getInstance().addEvento(evento);
